...
HT_tuning_base <- function( pinputexps, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    max_bin = 105L, # lo debo dejar fijo, no participa de la BO
    num_iterations = 9999, # un numero muy grande, lo limita early_stopping_rounds

    ###bagging_fraction = 1.0, # 0.0 < bagging_fraction <= 1.0
    bagging_fraction = 0.80, # 0.0 < bagging_fraction <= 1.0
    ###pos_bagging_fraction = 1.0, # 0.0 < pos_bagging_fraction <= 1.0
    pos_bagging_fraction = 0.80, # 0.0 < pos_bagging_fraction <= 1.0
    ...
    num_leaves = c( 90L, 2048L,  "integer" ),
    min_data_in_leaf = c( 220L, 2000L, "integer" )
  )
...
wf_septiembre <- function( pnombrewf )
{
  ...
  ###  hojas_por_arbol= 16,
    FErf_attributes_base( arbolitos= 110,
                          hojas_por_arbol= 30,
    datos_por_hoja= 1000,
    mtry_ratio= 0.2
  )
  ...
  ht <- HT_tuning_base( bo_iteraciones = 90 )  # iteraciones inteligentes
