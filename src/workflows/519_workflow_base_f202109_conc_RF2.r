...
HT_tuning_base <- function( pinputexps, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    max_bin = 60L, # lo debo dejar fijo, no participa de la BO
    num_iterations = 9999, # un numero muy grande, lo limita early_stopping_rounds

    ###bagging_fraction = 1.0, # 0.0 < bagging_fraction <= 1.0
    bagging_fraction = 0.9, # 0.0 < bagging_fraction <= 1.0
    bagging_freq = 1, ###### AGREGADO
    pos_bagging_fraction = 1.0, # 0.0 < pos_bagging_fraction <= 1.0
    neg_bagging_fraction = 1.0, # 0.0 < neg_bagging_fraction <= 1.0
    is_unbalance = FALSE, #
    scale_pos_weight = 1.0, # scale_pos_weight > 0.0

    ##(supuestamente no se usa en random forest)# drop_rate = 0.1, # 0.0 < neg_bagging_fraction <= 1.0 
    ##(supuestamente no se usa en random forest)# max_drop = 50, # <=0 means no limit
    ##(supuestamente no se usa en random forest)# skip_drop = 0.5, # 0.0 <= skip_drop <= 1.0
